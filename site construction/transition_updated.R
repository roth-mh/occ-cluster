...
.TfromR <- function(x, transitionFunction, directions, symm)
{
  ...
  # this only gets Values from the first Raster layer
  dataVals <- cbind(getValues(x[[1]])[adj[,1]],
                    getValues(x[[1]])[adj[,2]])
  for(lay in 2:nlayers(x)){
    dataVals <- cbind(dataVals,
                      getValues(x[[lay]])[adj[,1]],
                      getValues(x[[lay]])[adj[,2]])
  }
...
transition.BRICK <- function(x, transitionFunction, directions, symm=TRUE, intervalBreaks=NULL)
          {
            if(class(transitionFunction)=="character") 
            {
              if(transitionFunction != "barriers" & transitionFunction != "areas")
