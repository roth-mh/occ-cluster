...
runIDExp <- function(og_data, truth_df, covObj, geoClustAlpha=.8, occ_coeff, det_coeff){
  ...
  clustGeoSites_obj <- runClustGeoID(truth_df, covObj, og_data, occ_coeff, det_coeff, alpha=.8)
  CG6_occ_est <- clustGeoSites_obj$clust600$stats$mse$occ
  CG9_occ_est <- clustGeoSites_obj$clust900$stats$mse$occ
...
runSingleExp <- function(truth_df, covObj, og_data, n_sites, occ_coef = TRUE_OCC_COEFF, det_coef = TRUE_DET_COEFF){
  ...
  if(clustGeoAlpha_obj$clust5$mse$occ > 10){
